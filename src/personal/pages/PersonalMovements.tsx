    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 200, background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
      <div style={{ width: '100%', maxWidth: 480, background: '#0a1628', borderRadius: '1.5rem 1.5rem 0 0', border: '1px solid rgba(255,255,255,0.08)', borderBottom: 'none', padding: '1.25rem', maxHeight: '90dvh', overflowY: 'auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.25rem' }}>
          ...
          <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#475569', padding: '0.25rem', display: 'flex' }}><X size={18} /></button>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <PersonalInput label="Descripción *" value={description} onChange={e => setDescription(e.target.value)} placeholder="¿En qué?" />

          <PersonalSelect label="Cuenta *" value={accountId} onChange={e => setAccountId(e.target.value)}>
            <option value="">Seleccionar cuenta</option>
            {accounts.map(a => <option key={a.id} value={a.id}>{a.name} ({fmtMoney(a.current_balance)})</option>)}
          </PersonalSelect>

          {filteredCats.length > 0 && (
            <PersonalSelect label="Categoría" value={categoryId} onChange={e => setCategoryId(e.target.value)}>
          ...
          {error && <div style={{ padding: '0.625rem', background: 'rgba(248,113,113,0.08)', border: '1px solid rgba(248,113,113,0.25)', borderRadius: '0.5rem', color: '#f87171', fontSize: '0.8rem' }}>{error}</div>}
  ...
  return (
    <PageContainer>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <button onClick={() => setShowForm(true)} style={{ width: 36, height: 36, borderRadius: '0.75rem', background: 'rgba(52,211,153,0.12)', border: '1px solid rgba(52,211,153,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#34d399' }}>
          <Plus size={18} />
        </button>
      </div>
      ...
      {/* Filter tabs */}
      <div style={{ display: 'flex', gap: '0.375rem', padding: '0.25rem', background: 'rgba(255,255,255,0.04)', borderRadius: '0.75rem' }}>
        {(['all', 'income', 'expense'] as const).map(f => (
          <button key={f} onClick={() => setFilter(f)}
            style={{ flex: 1, padding: '0.5rem', borderRadius: '0.5rem', border: 'none', cursor: 'pointer', fontWeight: 600, fontSize: '0.8rem', background: filter === f ? 'rgba(255,255,255,0.08)' : 'transparent', color: filter === f ? '#f0f4ff' : '#475569', transition: 'all 0.15s' }}>
      ...
      {filtered.length === 0 ? (
        ...
        Object.entries(grouped)
          ...
          .map(([date, items]) => (
            <div key={date}>
              <SectionHeader title={new Date(date + 'T12:00:00').toLocaleDateString('es-AR', { weekday: 'long', day: 'numeric', month: 'long' })} />
              <Card>
                {items.map(tx => (
                  <TxRow
                    key={tx.id}

      ...
      <div className="modal-card" style={{ maxWidth: '560px' }}>
        ...
        <div className="modal-hdr">
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            <div className="page-hdr-icon" style={{ width: 36, height: 36 }}>
              <Tag size={18} style={{ color: 'var(--accent-primary)' }} />
            </div>
            <div>
              <h2 style={{ margin: 0, fontSize: '1rem', fontWeight: 700, color: 'var(--text-primary)' }}>{editing ? 'Editar oferta' : 'Nueva oferta'}</h2>
              <p style={{ margin: 0, fontSize: '0.72rem', color: 'var(--text-muted)' }}>Precio promocional temporal</p>
            </div>
          </div>
          <button onClick={onClose} className="icon-btn" aria-label="Cerrar"><X size={16} /></button>
        </div>

        <div className="modal-body-scroll" style={{ display: 'flex', flexDirection: 'column', gap: '1.125rem' }}>

          {/* Producto */}
          <div>
            <label className="label-caps">Producto *</label>
            <div style={{ position: 'relative' }}>
              <input className="form-control" value={searchQ} onChange={e => searchProducts(e.target.value)}
          ...
          {normalPrice > 0 && (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1rem' }}>
              <div>
                <label className="label-caps">Precio normal</label>
                <div className="form-control" style={{ color: 'var(--text-muted)', display: 'flex', alignItems: 'center' }}>
                  {fmtARS(normalPrice)}
                </div>
              </div>
              <div>
                <label className="label-caps">Precio de oferta *</label>
                <input className="form-control" style={{ color: '#22c55e', fontWeight: 700 }} type="number" min={1} max={normalPrice - 1}
                  value={offerPrice || ''} onChange={e => setOfferPrice(+e.target.value || 0)} placeholder="$ oferta" />
              </div>
              <div>
                <label className="label-caps">Descuento</label>
                <div className="form-control" style={{ color: discountPct > 0 ? '#22c55e' : 'var(--text-muted)', fontWeight: 700, display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
          ...
          {!normalPrice && editing && (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1rem' }}>
              <div>
                <label className="label-caps">Precio normal</label>
                <div className="form-control" style={{ color: 'var(--text-muted)' }}>{fmtARS(editing.normal_price)}</div>
              </div>
              <div>
                <label className="label-caps">Precio de oferta *</label>
                <input className="form-control" style={{ color: '#22c55e', fontWeight: 700 }} type="number" min={1}
                  value={offerPrice || ''} onChange={e => setOfferPrice(+e.target.value || 0)} placeholder="$ oferta" />
              </div>
              <div>
                <label className="label-caps">Descuento</label>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
            <div>
              <label className="label-caps">Fecha de inicio *</label>
              <input className="form-control" type="date" value={startDate} onChange={e => setStartDate(e.target.value)} />
            </div>
            <div>
              <label className="label-caps">Fecha de vencimiento *</label>
              <input className="form-control" type="date" value={endDate} min={startDate} onChange={e => setEndDate(e.target.value)} />
            </div>
          </div>
          ...
          <div>
            ...
            <textarea className="form-control" style={{ minHeight: 64, resize: 'vertical' }} value={notes} onChange={e => setNotes(e.target.value)} placeholder="Ej: Liquidación de stock, promo fin de semana..." />
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.75rem 1rem', borderRadius: '0.625rem', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}>
            <span style={{ color: 'var(--text-muted)', fontSize: '0.875rem' }}>Oferta activa</span>
        ...
        {error && (
          <div className="alert-inline alert-error" style={{ margin: '0 1.5rem' }}>
    ...
    <div className="page-shell">
      ...
      <div className="page-hdr">
        <div className="page-hdr-left">
          ...
          <div>
            <h1 className="page-hdr-title">Ofertas</h1>
            <p className="page-hdr-subtitle">Precios promocionales temporales</p>
          </div>
        </div>
        <div className="page-hdr-right">
          <button className="btn btn-primary btn-lift" onClick={() => { setEditing(null); setShowModal(true) }}>
      ...
      {(stats.last_day > 0 || stats.expired > 0) && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', marginBottom: '1.25rem' }}>
          {withStatus.filter(o => o._status === 'last_day').map(o => (
            <div key={o.id} className="alert-inline alert-warning" style={{ justifyContent: 'space-between' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
                <Clock size={14} style={{ flexShrink: 0 }} />
                <span><strong>Último día:</strong> oferta de <strong>{o.product?.name}</strong> vence hoy. Revisá si querés renovarla.</span>
              </div>
              <button className="btn btn-ghost btn-sm" onClick={() => handleRenew(o)}>
                <RotateCcw size={12} /> Renovar
              </button>
            </div>
          ))}
          {withStatus.filter(o => o._status === 'expired').slice(0, 3).map(o => (
            <div key={o.id} className="alert-inline alert-info">
              <AlertCircle size={14} style={{ flexShrink: 0 }} />
              <span>Oferta vencida: <strong>{o.product?.name}</strong> — venció el {fmtDate(o.end_date)}.</span>
            </div>
          ))}
        </div>
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: '1rem', marginBottom: '1.25rem' }}>
        {([
          ...
          <button key={s.key} onClick={() => setFilter(f => f === s.key ? 'all' : s.key as FilterKey)}
            className="stat-card"
            style={{ cursor: 'pointer', textAlign: 'left', width: '100%', border: filter === s.key ? `1px solid ${s.color}40` : undefined, background: filter === s.key ? `${s.color}08` : undefined }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.375rem', color: s.color }}>{s.icon}<span className="stat-card-label">{s.label}</span></div>
      ...
      <div className="filter-bar">
        <div style={{ position: 'relative', flex: 1, minWidth: 200 }}>
          ...
          <input className="form-control" style={{ paddingLeft: '2.25rem' }} placeholder="Buscar por producto, categoría, observación..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        </div>
        <div className="tabs">
          {([
            { key: 'all', label: 'Todas' },
      ...
      {loading ? (
        ...
        <EmptyState
          icon={Tag}
          title={searchTerm || filter !== 'all' ? 'Sin resultados' : 'Sin ofertas'}
          description={searchTerm || filter !== 'all' ? 'No hay resultados para los filtros seleccionados.' : 'No hay ofertas creadas todavía.'}
          action={filter === 'all' && !searchTerm ? { label: 'Crear primera oferta', onClick: () => { setEditing(null); setShowModal(true) } } : undefined}
        />
      ) : (
            ...
            <thead>
              <tr>
                {['Producto', 'Precio normal', 'Precio oferta', 'Descuento', 'Inicio', 'Vencimiento', 'Estado', 'Activa', ''].map(h => (
                  <th key={h} className="label-caps" style={{ textAlign: ['Precio normal','Precio oferta','Descuento'].includes(h) ? 'right' : 'left' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
                  ...
                  <tr key={o.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}
                    ...
                    <td>
                      <div style={{ display: 'flex', gap: '0.25rem', justifyContent: 'flex-end' }}>
                        <button className="icon-btn icon-btn-violet" aria-label="Editar" onClick={() => { setEditing(o); setShowModal(true) }}><Edit2 size={14} /></button>
                        {(st === 'expired' || st === 'last_day') && (
                          <button className="icon-btn icon-btn-primary" aria-label="Renovar" onClick={() => handleRenew(o)}><RotateCcw size={14} /></button>
                        )}
                        <button className="icon-btn icon-btn-danger" aria-label="Eliminar" onClick={() => handleDelete(o)}><Trash2 size={14} /></button>
                      </div>
                    </td>
                  </tr>
                )

-- ============================================================================
-- M7 HOTFIX - Check nuevo: vistas canonicas expuestas sin security_invoker.
--
-- NO TOCA NI UN SOLO DATO. Reemplaza finance_health_check_v2 con el mismo
-- cuerpo vigente y UN bloque AGREGADO: canonical_views_without_security_invoker.
--
-- POR QUE EXISTE ESTE CHECK
-- El hotfix 20260720120000 cerro un leak cross-tenant real: v_finance_pnl,
-- v_finance_sales_ledger y v_finance_product_margin estaban GRANTeadas a
-- authenticated SIN security_invoker, o sea corriendo con los privilegios de
-- postgres y salteando el RLS de todas sus tablas base. Un usuario del negocio
-- A leia las ventas, el COGS y los margenes del negocio B.
--
-- El Health Check v2 tenia 78 checks y NINGUNO miraba reloptions. Cubria el
-- search_path de las SECURITY DEFINER (7C.1/7C.1a) pero no el modo de
-- ejecucion de las VISTAS, que es el mismo tipo de defecto por otra puerta.
-- Sin este check, la proxima vista canonica que nazca sin security_invoker
-- vuelve a abrir el agujero y el tablero sigue en verde.
--
-- POR QUE ES GLOBAL Y NO POR NEGOCIO
-- Mira catalogo y grants, no filas. Sigue la conviccion de 7C §17: un hallazgo
-- de plataforma no debe pintar de rojo el health check de cada comercio. Va
-- detras de p_include_global + finance_hc_can_see_global (owner real,
-- verificado server-side), igual que los otros checks de `security`.
--
-- FALSOS POSITIVOS
-- Solo cuenta una vista si se cumplen las DOS condiciones: le falta
-- security_invoker Y es alcanzable por anon o authenticated. Una vista interna
-- sin grants a roles de cliente no puede filtrar nada hacia la app y no se
-- reporta. Por eso el predicado mira privilegios efectivos con
-- has_table_privilege y no la lista textual de grants.
--
-- El resto de la funcion se reproduce SIN CAMBIOS por copia binaria de
-- 20260714130000_m7_7e3_fix_bfe_income_duplicated_check.sql (que a su vez venia
-- de 7C). Se copio el archivo y se inserto un bloque: no se transcribio a mano.
-- Esa disciplina importa — 7E.3 documenta que reproducir el cuerpo a mano le
-- devolvio a las funciones un search_path viejo SIN pg_temp y deshizo la
-- barrera de 20260713310000. El DO del final vuelve a verificarlo.
-- ============================================================================
...
BEGIN
  ...
  -- M7 7E.3 — PREDICADO CORREGIDO.
  --
  -- El anterior contaba filas: count de BFE income > 1 por comprobante. Eso
  -- marcaba como error critico TODO cobro MIXTO, que genera legitimamente un
  -- espejo `revenue_collection_mirror` por cada medio de pago. Medido en
  -- produccion: 2 casos marcados, los DOS correctos
  --   1.000   = 500 efectivo + 500 transferencia
  --   110.000 = 100.000 efectivo + 10.000 transferencia
  -- En ambos: pagos = espejos = total. Cero impacto en P&L. Era un falso
  -- positivo, y encima critical, sobre una funcionalidad que el POS soporta:
  -- iba a dispararse en cada venta mixta.
  --
  -- El invariante economico real es que el espejo de ingreso NO PUEDE SUPERAR
  -- lo efectivamente cobrado.
  --
  -- Se compara contra los PAGOS y no contra total_bruto: el recargo al cliente
  -- hace que lo cobrado supere legitimamente al bruto. Medido: 8 comprobantes
  -- donde bfe = total_cobrado = pagos y el "exceso" sobre el bruto es
  -- exactamente el recargo de tarjeta. Usar total_bruto habria cambiado un
  -- falso positivo de 2 por otro de 8.
  --
  -- Se comparan ACTIVOS contra ACTIVOS: se excluyen los espejos revertidos
  -- (reversed_at) y los pagos reemplazados (replaced_at).
  --
  -- Solo evalua cuando HAY cobro que espejar. Un espejo sin ningun pago es otro
  -- invariante (no es duplicacion) y no le corresponde a este check.
  SELECT count(*), COALESCE(SUM(exceso),0) INTO n, m FROM (
    SELECT b.reference_comprobante_id,
           SUM(b.amount_ars) - COALESCE(p.pagos,0) AS exceso
      FROM business_finance_entries b
      JOIN (SELECT comprobante_id, SUM(amount_ars) AS pagos, count(*) AS n_pagos
              FROM comprobante_payments
             WHERE replaced_at IS NULL
             GROUP BY 1) p ON p.comprobante_id = b.reference_comprobante_id
     WHERE b.business_id=v_biz AND b.type='income' AND b.amount_ars > 0
       AND b.reference_comprobante_id IS NOT NULL
       AND b.reversed_at IS NULL
       AND p.n_pagos > 0
  ...
  v_c := v_c || finance_hc_mk('bfe_income_duplicated','accounting_classification','Ingreso duplicado',
    CASE WHEN n=0 THEN 'pass' ELSE 'fail' END, 'critical', n, m,
    CASE WHEN n=0 THEN 'Ningun comprobante contabiliza mas ingreso del que cobro (los cobros mixtos son validos)'
         ELSE n||' comprobante(s) con ingreso espejado por encima de lo cobrado' END);
  ...
  -- HOTFIX 20260719 — Vistas expuestas que corren con privilegios del OWNER.
  -- Una vista sin security_invoker se evalua como su owner (postgres), que no
  -- esta sujeto a RLS: devuelve filas de TODOS los negocios a cualquier rol con
  -- SELECT. Es el mismo defecto de clase que `secdef_without_search_path`, pero
  -- por la puerta de las vistas.
  --
  -- Se exige la conjuncion (sin invoker) AND (alcanzable por cliente): una
  -- vista interna sin grants no puede filtrar hacia la app. Asi el check no
  -- genera ruido sobre vistas de uso administrativo.
  SELECT count(*) INTO n
    FROM pg_class c
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
           'vista', x.relname,
           'anon', x.anon_sel,
           'authenticated', x.auth_sel) ORDER BY x.relname), '[]'::jsonb)
    INTO d
    FROM (SELECT c.relname,
           ...
           WHERE ns.nspname = 'public'
             ...
             AND NOT COALESCE(c.reloptions::text[] @> ARRAY['security_invoker=true'], false)
             AND (has_table_privilege('anon', c.oid, 'SELECT')
               OR has_table_privilege('authenticated', c.oid, 'SELECT'))) x;

  v_c := v_c || finance_hc_mk('canonical_views_without_security_invoker','security',
    'Vistas expuestas sin security_invoker',
    CASE WHEN n=0 THEN 'pass' ELSE 'fail' END, 'critical', n, 0,
    CASE WHEN n=0 THEN 'Toda vista alcanzable por anon/authenticated se evalua con los privilegios de quien consulta (RLS activo)'
         ELSE n||' vista(s) alcanzables por anon/authenticated corren con los privilegios de su owner y saltean RLS: exponen datos de todos los negocios' END,
    jsonb_build_object(
      'vistas', d,
      'regla', 'una vista sin security_invoker se evalua como su owner; si el owner no esta sujeto a RLS, el aislamiento por business_id desaparece',
      'remediacion', 'ALTER VIEW public.<vista> SET (security_invoker = true)'));
  END IF;
...
-- ============================================================================
-- REPONER LA BARRERA pg_temp (M7 7C.1a)
--
-- Esta migracion reproduce el cuerpo de 7C, que es ANTERIOR a la migracion
-- 20260713310000_m7_7c1a_pgtemp_barrier_all_secdef. Al hacer CREATE OR REPLACE
-- con el texto original, las dos funciones SECURITY DEFINER que define vuelven
-- a quedar con el search_path viejo, SIN pg_temp — o sea, esta migracion
-- deshacia la barrera para ellas.
--
-- No es un detalle cosmetico: omitir pg_temp no lo excluye del search_path, lo
-- pone PRIMERO (doc PostgreSQL 5.9.3). Es exactamente el vector de shadowing
-- por tabla temporal que cerro 7C.1a.
--
-- Lo detectaron las suites etapa7_7c1 (PT3) y etapa7_7e1 (PC14) al correr
-- despues de aplicar esta migracion en local. Se repone aca para que la
-- migracion sea correcta por si sola, sin depender del orden de aplicacion.
-- ============================================================================
ALTER FUNCTION public.finance_health_check_v2(p_business_id uuid, p_include_global boolean)
  SET search_path = public, pg_temp;

ALTER FUNCTION public.finance_hc_can_see_global(p_business_id uuid)
  ...
  IF EXISTS (
    SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
     WHERE n.nspname = 'public' AND p.prosecdef
       AND p.proname IN ('finance_health_check_v2','finance_hc_can_see_global')
       AND NOT EXISTS (SELECT 1 FROM unnest(coalesce(p.proconfig,'{}'::text[])) x
                        WHERE x LIKE '%pg_temp%')
  ) THEN
    RAISE EXCEPTION '7E.3: alguna funcion del health check quedo sin pg_temp en el search_path';
  END IF;
  RAISE NOTICE '7E.3 OK: predicado corregido y barrera pg_temp repuesta';
END $barrera$;

...
--   * comprobante FISCAL (factura_a / factura_c / nota_credito)
--       -> PV = arca_config.punto_venta, resuelto server-side. El payload del
--          cliente se descarta por completo.
--       -> si se pidio emitir en ARCA y no hay configuracion: fail-closed
...
BEGIN
  ...
  BEGIN
    ...
    -- factura o de una NC: el CAE se pide SIEMPRE con arca_config.punto_venta
    ...
    IF v_tipo_es_fiscal THEN
      SELECT punto_venta INTO v_arca_pv
        FROM arca_config WHERE business_id = p_business_id;

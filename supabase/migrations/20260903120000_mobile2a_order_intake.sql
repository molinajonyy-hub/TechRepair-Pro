-- MOBILE-2A · Recepción mobile-first
...
CREATE TABLE IF NOT EXISTS private.order_device_access_audit (
  ...
  actor_id uuid,
  action text NOT NULL CHECK (action IN ('migrated','stored','replaced','revealed','deleted')),
  created_at timestamptz NOT NULL DEFAULT now()
);
...
CREATE OR REPLACE FUNCTION private.mobile2a_store_access(
  ...
  p_audit_action text DEFAULT 'stored')
...
BEGIN
  ...
  INSERT INTO private.order_device_access_audit(order_id,business_id,actor_id,action)
  VALUES (p_order_id,p_business_id,p_actor,p_audit_action);
END;
$$;
REVOKE ALL ON FUNCTION private.mobile2a_store_access(uuid,uuid,uuid,text,text,text)
  FROM PUBLIC, anon, authenticated, service_role;

-- Migra cualquier valor legacy antes de bloquear para siempre el texto plano.
DO $$
...
BEGIN
  FOR r IN SELECT id,business_id,created_by,device_password FROM public.orders
    ...
    v_mode := CASE WHEN r.device_password LIKE 'pattern:%' THEN 'pattern'
                   WHEN r.device_password LIKE 'pin:%' THEN 'pin' ELSE 'password' END;
    v_secret := CASE WHEN position(':' in r.device_password)>0
                     THEN substring(r.device_password from position(':' in r.device_password)+1)
                     ELSE r.device_password END;
    PERFORM private.mobile2a_store_access(r.id,r.business_id,r.created_by,v_mode,v_secret,'migrated');
    UPDATE public.orders SET access_mode=v_mode, device_password=NULL WHERE id=r.id;
  END LOOP;
  UPDATE public.orders SET device_password=NULL WHERE device_password IS NOT NULL;
END $$;

ALTER TABLE public.orders DROP CONSTRAINT IF EXISTS orders_device_password_retired_check;
ALTER TABLE public.orders ADD CONSTRAINT orders_device_password_retired_check CHECK (device_password IS NULL);
COMMENT ON COLUMN public.orders.device_password IS
  'RETIRADA MOBILE-2A: siempre NULL. El secreto vive cifrado en Supabase Vault y sólo sale por RPC auditado.';
...
BEGIN
  ...
  IF v_mode IN ('pin','pattern','password') THEN
    PERFORM private.mobile2a_store_access(v_order,v_business,v_actor,v_mode,p_access_secret,'stored');
  END IF;
...
BEGIN
  ...
  UPDATE public.orders SET access_mode=p_mode,device_password=NULL,updated_at=now() WHERE id=p_order_id;
END; $$;
...
BEGIN
  ...
  INSERT INTO private.order_device_access_audit(order_id,business_id,actor_id,action)
  VALUES(p_order_id,v_business,v_actor,'revealed');
  RETURN v_value;
END; $$;
...
SET search_path = pg_catalog, pg_temp
AS $$
DECLARE v_actor uuid:=auth.uid(); v_business uuid; v_secret_id uuid;
BEGIN
  IF NOT public.current_user_can('device_access_secret') THEN RAISE EXCEPTION 'FORBIDDEN_DEVICE_ACCESS' USING ERRCODE='42501'; END IF;
  v_business:=private.mobile2a_business_for_actor(v_actor);
  SELECT s.vault_secret_id INTO v_secret_id FROM private.order_device_access_secrets s
   WHERE s.order_id=p_order_id AND s.business_id=v_business FOR UPDATE;
  ...
  UPDATE public.orders SET access_mode='none',device_password=NULL,updated_at=now() WHERE id=p_order_id AND business_id=v_business;
  INSERT INTO private.order_device_access_audit(order_id,business_id,actor_id,action)
  VALUES(p_order_id,v_business,v_actor,'deleted');
END; $$;

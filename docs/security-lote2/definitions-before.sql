-- READ-ONLY EVIDENCE: pg_get_functiondef from production, CRLF normalized. Do NOT execute.
...
CREATE OR REPLACE FUNCTION public.user_can_allocate_payments(p_business_id uuid, p_user_id uuid)
 RETURNS boolean
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'pg_temp'
...
CREATE OR REPLACE FUNCTION public.user_can_reverse_allocations(p_business_id uuid, p_user_id uuid)
 RETURNS boolean
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'pg_temp'
...
CREATE OR REPLACE FUNCTION public.user_can_view_order_amounts(p_business_id uuid, p_user_id uuid)
 RETURNS boolean
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'pg_temp'

...
-- ==== private.update_inventory_dollar_prices ====
CREATE OR REPLACE FUNCTION private.update_inventory_dollar_prices(p_business_id uuid, p_new_rate numeric)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
-- ==== public.annul_comprobante_atomic ====
...
-- ==== public.create_comprobante_checkout_atomic ====
...
AS $function$
      BEGIN
        PERFORM private.require_action_authority(p_business_id, 'comprobantes', NULL, NULL);
        RETURN private.create_comprobante_checkout_atomic(p_business_id,p_idempotency_key,p_request_hash,p_payload);
      END;
      $function$

-- ==== public.create_quick_inventory_purchase_atomic ====
CREATE OR REPLACE FUNCTION public.create_quick_inventory_purchase_atomic(p_business_id uuid, p_idempotency_key text, p_supplier_id uuid, p_supplier_name text, p_invoice text, p_date date, p_payment_method text, p_total_ars numeric, p_paid_ars numeric, p_items jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'pg_temp'
...
-- ==== public.preview_missing_stock_movements ====
...
CREATE OR REPLACE FUNCTION public.sync_inventory_stock_alias()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'

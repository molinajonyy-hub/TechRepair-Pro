...
SELECT ('e0800000-0000-0000-0000-' || lpad(i::text,12,'0'))::uuid,
       'sec08e-'||i||'@test.invalid', now() FROM generate_series(1,9) i;
...
SELECT u.id,u.id,
  ...
  (ARRAY['owner','tech','viewer','cashier','owner','admin','tech','sales','manager'])[x.i],
  x.i<>6,u.email,
...
FROM generate_series(1,9) x(i) JOIN auth.users u
